...
uint32_t static audio_joystick_val;
...
void TIMER0_IRQHandler(void) 
{
  ...
  LPC_TIM0->IR = 1;                         /* Clear Interrupt Flag */
	audio_joystick_val = get_button();
	if(audio_joystick_val == KBD_DOWN){
		NVIC_SystemReset();
	}
}
...
int audio_main (void)
{
  volatile uint32_t pclkdiv, pclk;
	GLCD_SetBackColor(Green);
        	...
        	GLCD_DisplayString (1, 0, 1, "   Welcome to audio  	");
        	GLCD_DisplayString (2, 0, 1, "   Press KBD down to exit  	");
